...
> can't start lowercase). Same convention Apple uses for iPhone/`IPhone`.
...
# Publish (what testers run):
dotnet build-server shutdown                   # ← if publish fails with MSB4018/OOM
dotnet publish src/Igloo.App/Igloo.App.csproj -c Release -r win-x86 `
  --self-contained true -p:PublishProfile=FolderProfile
# → src/Igloo.App/bin/Release/net8.0-windows10.0.19041.0/publish/win-x86/
```
...
- **Line endings are load-bearing.** Everything that crosses to Linux (kickstart,
  ...
  once broke Fedora's kickstart include. Don't "fix" the normalizers.
