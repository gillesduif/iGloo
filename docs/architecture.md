...
- **Plugins never touch the disk.** They *declare* (boot spec) and *render* (config,
  agent payload). Generic preparation services perform Windows disk work:
  `DirectInstallService` for legacy staging, and the opt-in
  `IInstallationTargetPreparer` for explicit creation and identity capture.
- The **manifest** (`migration-manifest.json`) is generated by the wizard and read
  downstream. The opt-in preparer can append a verified installation-target claim
  after re-reading the actual created partition. See the
  [target identity contract](reference/installation-target-identity.md) for strict
  GUID/geometry resolution across reboot, versioning and ownership requirements.
  Legacy `PreCreateRootPartition` does not establish ownership. Deepin requests
  strong identity but remains blocked from installation and legacy preparation.
